...
// the abv is for users that paid but below is for users that paid once the can still chat even if their merge has expired
import React, { useState, useEffect } from "react";
import {
  Box,
  Button,
...
const MergeScreen = () => {
  ...
  const handlePlanClick = async (planKey) => {
    ...
    // ✅ USER HAS PAID → GO DIRECTLY TO CHAT
    if (canChat) {
      navigate(`/chat/${member1}/${member2}`);
      return;
    }
    ...
    if (plan.amount === 0) {
      try {
        ...
        if (res.data.match) {
          ...
          setErrorMessage(
            "Free merge saved. You can chat with this match this month."
          );
        }
      } catch (err) {
        setErrorMessage(err?.response?.data?.message || "Free merge failed.");
