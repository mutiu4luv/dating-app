...
//       case "Messages":
//         navigate("/messages");
//         setUnreadCount(0); // Reset badge count when viewing messages
//         break;
//       case "Profile":
//         navigate("/profile");
//         break;
...
const Navbar = () => {
  ...
  useEffect(() => {
    ...
    return () => window.removeEventListener("unreadReset", handleUnreadReset);
  }, []);
  useEffect(() => {
    const resetUnreadCount = () => setUnreadCount(0);
    window.addEventListener("unreadReset", resetUnreadCount);
    return () => window.removeEventListener("unreadReset", resetUnreadCount);
  }, []);
  ...
  useEffect(() => {
    const checkForceRefresh = () => {
      if (localStorage.getItem("forceUnreadRefresh") === "true") {
        setUnreadCount(0);
        localStorage.removeItem("forceUnreadRefresh");
      }
    };
    checkForceRefresh();
    window.addEventListener("storage", checkForceRefresh);
    return () => window.removeEventListener("storage", checkForceRefresh);
  }, []);
